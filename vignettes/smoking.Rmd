...
de.sns_eset <- deAna(sns.eset, de.method = 'edgeR')

design = model.matrix(~smokingstatus, data=data.frame(sample_data(NYC_HANES.sns)))

otu_table.voom <- limma::voom(assay(de.sns_eset), design = design, plot = FALSE)
assay(de.sns_eset) <- otu_table.voom$E
class(assay(de.sns_eset)) <- "matrix"
...
sns.gsea.res <- sbea("gsea", de.sns_eset, sns_my.gs)
...
expr = assay(de.sns_eset)
expr = expr[rowData(de.sns_eset)$ADJ.PVAL <= 0.05,]
...
rownames(expr) <- paste(data.frame(tax_table(NYC_HANES)[rownames(expr),])$Genus, ' OTU')
...
#### GSEA result tested in boxplot
```{r echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, paged.print=FALSE}
ind <- which.min(rowData(de.sns_eset)$ADJ.PVAL)
otuname <- rownames(de.sns_eset)[ind]
smoker.exprs <- assay(de.sns_eset)[ind,de.sns_eset$GROUP==0]
nsmoker.exprs <- assay(de.sns_eset)[ind,de.sns_eset$GROUP==1]
...
### ORA
```{r echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, paged.print=FALSE}
sns_ora.res <- EnrichmentBrowser::sbea("ora", de.sns_eset, my.gs, perm=1000)
...
de.sh_eset <- deAna(subeset, de.method = 'edgeR')

otu_table.voom <- limma::voom(assay(de.sh_eset), design = sh_design, plot = FALSE)
assay(de.sh_eset) <- otu_table.voom$E
class(assay(de.sh_eset)) <- "matrix"
...
sns.nc.my.gsva <- GSVA::gsva(sns.nc.eset, sns.nc.my.gs, parallel.sz = 1)
...
otu_table.voom <- limma::voom(assay(de.hookah_eset), design = hookah_design , plot = FALSE)
assay(de.hookah_eset) <- otu_table.voom$E
class(assay(de.hookah_eset)) <- "matrix"
...
otu_table.voom <- limma::voom(assay(de.ecig_eset), design = ecig_design, plot = FALSE)
assay(de.ecig_eset) <- otu_table.voom$E
class(assay(de.ecig_eset)) <- "matrix"
...
otu_table.voom <- limma::voom(assay(de.cigars_eset), design = cigars_design, plot = FALSE)
assay(de.cigars_eset) <- otu_table.voom$E
class(assay(de.cigars_eset)) <- "matrix"
