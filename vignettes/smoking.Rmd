...
```
## GSEA hookah smokers vs never smokers
...
# NYC_HANES.hookah <- prune_taxa((NYC_HANES.hookah %>% otu_table  %>% rowSums())>30,NYC_HANES.hookah)
meta <- sample_data(NYC_HANES.hookah)
levels(meta$smokingstatus) <- c('Never','Alternative')

pdata <- new("AnnotatedDataFrame",meta)
group = meta$smokingstatus

design = model.matrix(~smokingstatus, data.frame(meta))

# d <- edgeR::DGEList(otu_table(NYC_HANES.hookah), group = group)
...
de.eset <- EnrichmentBrowser::de.ana(eset, de.method = 'edgeR')
...
## ORA hookah smokers vs never smokers
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
ora.res <- EnrichmentBrowser::sbea("ora", de.eset, my.gs, perm=0)
EnrichmentBrowser::gs.ranking(ora.res, signif.only=FALSE)
```

## GSEA e-cigarette smokers vs never smokers
```{r echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, paged.print=FALSE}
NYC_HANES.ecig <- NYC_HANES %>% subset_samples(., smokingstatus %in% c("Never smoker") | E_CIGARETTES == 1)
NYC_HANES.ecig %<>% prune_taxa((NYC_HANES.ecig %>% otu_table  %>% rowSums())>30,.)
meta <- sample_data(NYC_HANES.ecig)
pdata <- new("AnnotatedDataFrame",meta)

group = meta$smokingstatus
design = model.matrix(~ smokingstatus, data = data.frame(sample_data(NYC_HANES.ecig)))

d <- edgeR::DGEList(otu_table(NYC_HANES.ecig), group = sample_data(NYC_HANES.ecig)$smokingstatus)
d <- edgeR::calcNormFactors(d)
class(d$counts) <- "matrix"
otu_table.voom <- limma::voom(d, design = design, plot = FALSE)

eset <- Biobase::ExpressionSet(assayData = otu_table.voom$E, phenoData = pdata)
eset$GROUP <- ifelse(eset$smokingstatus == "Alternative smoker",1,0)
...
## ORA e-cigarette smokers vs never smokers
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
ora.res <- EnrichmentBrowser::sbea("ora", de.eset, my.gs, perm=0)
EnrichmentBrowser::gs.ranking(ora.res, signif.only=FALSE)
```

## GSEA cigarillos smokers vs never smokers
```{r echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, paged.print=FALSE}
NYC_HANES.cigars <-NYC_HANES %>% subset_samples(., smokingstatus %in% c("Never smoker") | CIGARS_CIGARILLOS == 1)
NYC_HANES.cigars %<>% prune_taxa((NYC_HANES.cigars %>% otu_table  %>% rowSums())>30,.)
meta <- sample_data(NYC_HANES.cigars)
pdata <- new("AnnotatedDataFrame",meta)

group = sample_data(NYC_HANES.cigars)$smokingstatus
design = model.matrix(~smokingstatus, data=data.frame(sample_data(NYC_HANES.cigars)))

d <- edgeR::DGEList(otu_table(NYC_HANES.cigars), group = sample_data(NYC_HANES.cigars)$smokingstatus)
d <- edgeR::calcNormFactors(d)
class(d$counts) <- "matrix"
otu_table.voom <- limma::voom(d, design = design, plot = FALSE)
...
## ORA cigarillos smokers vs never smokers
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
ora.res <- EnrichmentBrowser::sbea("ora", de.eset, my.gs, perm=0)
EnrichmentBrowser::gs.ranking(ora.res, signif.only=FALSE)
```
...
eset <- Biobase::ExpressionSet(assayData = otu_table(NYC_HANES.sns), phenoData = pdata)
