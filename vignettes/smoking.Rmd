...
res.crude <- DESeq2::results(dds, contrast = c("smokingstatus","Cigarette","Never smoker"))
res.filtered <- res[!is.na(res.crude$padj),]
...
if(all.equal(rownames(res.filtered.conf), rownames(tt))){
  res.tax.conf <- cbind(as.data.frame(res.filtered.conf), as.matrix(tt))
}else{
...
compardat <- data.frame(crude = res.crude$log2FoldChange, 
...
sample_data(NYC_HANES)$smokingstatus <- relevel(sample_data(NYC_HANES)$smokingstatus,"Never smoker")
dasmoking.crude <- get_edgeR_results(~ smokingstatus, coef = 2, alph = 0.05, method = "BH")
dasmoking.crude$table <- dasmoking.crude$table[sort(dasmoking.crude$table %>% rownames),]
```

```{r edgeR_univ, echo=FALSE, fig.width=6.5}
dasmoking.crude$table$Genus <- as.character(dasmoking.crude$table$Genus)
dasmoking.crude$table[c("New.CleanUp.ReferenceOTU8965","New.ReferenceOTU179"), 'Genus'] <- 'uncultured'
dasmoking.crude$table['New.ReferenceOTU72','Genus'] <- 'Family XIII'
dasmoking.crude$table$Genus[is.na(dasmoking.crude$table$Genus)] <- 'unclassified'
ord <- order(dasmoking.crude$table[['Phylum']], dasmoking.crude$table$logFC, decreasing=TRUE)
dasmoking.crude$table$Genus <- factor(dasmoking.crude$table$Genus, levels = unique(dasmoking.crude$table$Genus[ord]))
phyla_colors <- c('#00B2FFFF','#FF9900FF','#FF0000FF','#0DFF00FF','#00FF9FFF','#A600FFFF','#B9FF00FF','#FF00ACFF','#0006FFFF','#ababab')


p <-
  ggplot2::ggplot(dasmoking.crude$table, ggplot2::aes(x=logFC, y=Genus, color=Phylum)) +
...
dasmoking.crude <- get_edgeR_results(~ smokingstatus, coef = 2, alph = 2, method = "BH")
dasmoking.crude$table <- dasmoking.crude$table[sort(dasmoking.crude$table %>% rownames),]
...
if(identical(all.equal(rownames(da.adj), rownames(dasmoking.crude)), TRUE)){
  compardat <- data.frame(crude = dasmoking.crude$table$logFC, 
...
A function for merging smoking and cotenine edgeR results:
```{r}
mergeResults <- function(obj.smoking, obj.cotenine) {
  intersect.filters <-
    intersect(rownames(obj.smoking), rownames(obj.cotenine))
  intersect.filters <- sort(intersect.filters)
  obj.smoking <- obj.smoking[intersect.filters,]
  obj.cotenine <- obj.cotenine[intersect.filters,]
  stopifnot(all.equal(rownames(obj.smoking), rownames(obj.cotenine)))
  ##
  res <-
    data.frame(
      logFC_smoking = obj.smoking$table$logFC,
      logFC_cotenine = obj.cotenine$table$logFC,
      FDR_smoking = obj.smoking$table$FDR,
      FDR_cotenine = obj.cotenine$table$FDR,
...
```{r}
plotSmokingCotenineComparison <-
  function(obj.smoking,
           obj.cotenine,
           labtext = "Crude",
           FDRcutoff = 1) {
    dataset <- mergeResults(obj.smoking, obj.cotenine)
    dataset <- dataset[dataset$FDR_smoking < FDRcutoff,]
    corr.result <- with(dataset,
                        cor.test(logFC_smoking, logFC_cotenine))
    p <- ggplot(dataset) +
      theme_bw() +
      ...
      geom_point(aes(logFC_smoking, logFC_cotenine, color = FDR_smoking <= 0.05),
                 size = 3) +
      scale_colour_manual(values = setNames(c('black', 'grey'), c(TRUE, FALSE)), guide =
                            FALSE) +
      geom_smooth(
        aes(logFC_smoking, logFC_cotenine),
        method = lm,
        se = FALSE,
        color = "red"
      ) +
...
dacot.crude <- get_edgeR_results(~ COTININE, 
                                 pseq = subset_samples(NYC_HANES, smokingstatus %in% c("Secondhand")), 
...
Correlation between crude smoking and cotenine coefficients, all OTUs:
```{r}
with(edger.smoker_secondhand_crude, 
     cor.test(logFC_smoking, logFC_cotenine))
```

Correlation between crude smoking and cotenine coefficients, FDR < 0.05 in smoking:
```{r}
with(filter(edger.smoker_secondhand_crude, FDR_smoking < 0.05),
  cor.test(logFC_smoking, logFC_cotenine))
```
...
```{r}
p <-
  plotSmokingCotenineComparison(dasmoking.crude,
                                dacot.crude,
                                labtext = "Crude",
                                FDR = 1)
p
...
summary(edger.smoker_secondhand_adjusted$logFC_cotenine < 0.05)
```

Correlation between adjusted smoking and cotenine coefficients, all OTUs:
```{r}
with(edger.smoker_secondhand_adjusted, 
     cor.test(logFC_smoking, logFC_cotenine))
```

Correlation between adjusted smoking and cotenine coefficients, FDR < 0.05 in smoking:
```{r}
