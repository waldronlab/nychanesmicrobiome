...
group = sample_data(NYC_HANES.ecig)$smokingstatus
...
eset <- ExpressionSet(assayData = otu_table.voom$E, phenoData = pdata)
eset$GROUP <- ifelse(eset$smokingstatus == "Alternative smoker",0,1)
de.eset <- EnrichmentBrowser::de.ana(eset)
...
```{r echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, paged.print=FALSE}
NYC_HANES.hookah <-NYC_HANES %>% subset_samples(., smokingstatus %in% c("Never smoker") | HOOKAH_PIPE == 1)
NYC_HANES.hookah %<>% prune_taxa((NYC_HANES.hookah %>% otu_table  %>% rowSums())>30,.)
meta <- sample_data(NYC_HANES.hookah)
pdata <- new("AnnotatedDataFrame",meta)

group = sample_data(NYC_HANES.hookah)$smokingstatus
design = model.matrix(~smokingstatus, data=data.frame(sample_data(NYC_HANES.hookah)))

d <- edgeR::DGEList(otu_table(NYC_HANES.hookah), group = sample_data(NYC_HANES.hookah)$smokingstatus)
d <- edgeR::calcNormFactors(d)
class(d$counts) <- "matrix"
otu_table.voom <- limma::voom(d, design = design, plot = FALSE)
